...
function Login() {
    ...
    const submitHandler = async (e) => {
        e.preventDefault()
        await axios.post('http://localhost:8080/api/v1/center/login', { codeCt: info.codeCt, password: info.password })
            .then((res) => {
                console.log(res.data.codeCt)
                localStorage.setItem('token', res.data.token)
                localStorage.setItem('center' , res.data.codeCt)
                setInfo({
                    codeCt: '',
                    password: ''
                })
    ...
    return (
        <form className="form" onSubmit={(e) => { submitHandler(e) }}>
            <p className="form-title">Sign in to your center</p>
